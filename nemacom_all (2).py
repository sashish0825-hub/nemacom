...
def run_app():
    ...
    with st.sidebar:
        ...
        pal = st.selectbox("Colour palette", list(npl.PALETTES.keys()),
                           help="The first three are safe for colour-vision "
                                "deficiency. Many journals now expect this.")
        st.write(" ".join(f":{'red' if False else 'gray'}[]" for _ in range(0)))
        st.markdown(" ".join(
...
""")
    ...
    with tabs[1]:
        ...
        if grp is None:
            ...
            st.download_button("Download this figure (PNG)",
                               _fig_bytes(fig, dpi), f"{m}.png", "image/png")
            plt.close(fig)
            if letters:
                st.caption("Bars sharing a letter do not differ (Tukey HSD, "
                           "p > 0.05). Points are individual samples.")
    ...
    with tabs[2]:
        ...
        npl.apply_style(fsize)
        cols = npl.palette(pal, len(pd.unique(grp)) if grp is not None else 1)
        fig, ax = plt.subplots(figsize=(6.5, 6))
        if grp is None:
            ax.scatter(fn["SI"], fn["EI"], s=90, color=cols[0], edgecolor="w")
        else:
            for i, lev in enumerate(pd.unique(grp)):
                m_ = grp.reindex(fn.index) == lev
                ax.scatter(fn.loc[m_, "SI"], fn.loc[m_, "EI"], s=90,
                           color=cols[i], edgecolor="w", label=lev)
        ...
        for x_, y_, t_ in [(6,94,"A Maturing"),(74,94,"B Structured"),
                           (6,3,"D Degraded"),(74,3,"C Disturbed")]:
            ax.text(x_, y_, t_, color="grey", fontsize=8)
        ax.set_title("Faunal profile (Ferris et al. 2001)", loc="left",
                     fontweight="bold")
        st.pyplot(fig, use_container_width=False)
        st.download_button("Download faunal profile (PNG)", _fig_bytes(fig, dpi),
                           "faunal_profile.png", "image/png")
        plt.close(fig)
        ...
        if nsh is None:
            ...
            if grp is not None:
                ...
                npl.bar_with_error(nsh["NSH"], grp, err=errtype, pal=pal,
                                   ...
                                   letters=npl.compact_letters(tk) if not tk.empty
                                   else None, ax=ax)
                ax.axhline(15, ls="--", c="#C44E52", lw=1)
                ax.axhline(25, ls="--", c="#55A868", lw=1)
                ax.text(ax.get_xlim()[1], 15, " degraded", va="center",
                        fontsize=7, color="#C44E52")
                ax.text(ax.get_xlim()[1], 25, " well-functioning", va="center",
                        fontsize=7, color="#55A868")
                st.pyplot(fig, use_container_width=False)
                st.download_button("Download NSH figure (PNG)",
                                   _fig_bytes(fig, dpi), "nsh.png", "image/png")
                plt.close(fig)
            else:
            ...
            st.caption("Ghaderi et al. state the index requires further "
                       "calibration, and their abundance bands come from global "
                       "biome ranges in which tropical cropland is thinly "
                       "represented. These diagnostics show whether the index is "
                       "discriminating usefully in YOUR system.")
            st.write("**Do the subscores vary?** A subscore with range 0 is "
                     "constant across all your samples and contributes nothing.")
            st.dataframe(npl.subscore_contributions(nsh).round(2),
                         use_container_width=True)
            if grp is not None:
                disc = npl.nsh_discrimination(nsh, grp)
                cA, cB, cC = st.columns(3)
                cA.metric("Scale used", f"{disc.get('pct_of_scale_used',0):.0f}%",
                          help="Of the full 8-32 range")
                cB.metric("eta squared", f"{disc.get('eta_squared',float('nan')):.2f}",
                          help="Variance in NSH explained by treatment")
                cC.metric("Categories occupied",
                          f"{disc.get('n_categories_occupied',0)} of 3")
                if disc.get("pct_of_scale_used", 100) < 35:
                    st.warning("Your samples occupy a narrow part of the NSH "
                               "scale. That is exactly the recalibration problem "
                               "the authors flagged - worth reporting rather "
                               "than working around.")
    ...
    with tabs[4]:
        ...
        if fps is None:
            ...
            npl.apply_style(fsize)
            fig, ax = plt.subplots(figsize=(6.5, 6))
            npl.footprint_profile(fn, fps, groups=grp, pal=pal, ax=ax)
            st.pyplot(fig, use_container_width=False)
            st.download_button("Download footprint profile (PNG)",
                               _fig_bytes(fig, dpi), "footprints.png", "image/png")
            plt.close(fig)
    ...
    with tabs[5]:
        ...
        if grp is None:
            ...
            c1, c2 = st.columns(2)
            with c1:
                st.write("**Tukey HSD** (parametric)")
                st.dataframe(npl.tukey_posthoc(combined[which], grp).round(4),
                             use_container_width=True)
            with c2:
                st.write("**Dunn** (non-parametric, Bonferroni)")
                st.dataframe(npl.dunn_posthoc(combined[which], grp).round(4),
                             use_container_width=True)
...
def _under_streamlit():
    ...
    Streamlit executes the script as a module named "__main__"
    (script_runner.py: module = self._new_module("__main__")), so the usual
    __name__ check cannot distinguish it from a plain `python file.py`. The
    reliable signal is an active ScriptRunContext."""
    try:
